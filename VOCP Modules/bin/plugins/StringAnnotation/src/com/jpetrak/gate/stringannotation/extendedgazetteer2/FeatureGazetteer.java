  ...
  protected void addLookupsToAnn(Annotation ann, Iterator<Lookup> lookups, boolean overwrite) {
    ...
    while(lookups.hasNext()) {
      ...
      if(overwrite) {
        ...
        FeatureMap tmpfm = Factory.newFeatureMap();
        gazStore.addLookupListFeatures(tmpfm, lookup);
        gazStore.addLookupEntryFeatures(tmpfm, lookup);   
        // now add only features not already in fm
        for(Object key : tmpfm.keySet()) {
          if(fm.get(key) == null) {
            fm.put(key, tmpfm.get(key));
          }
        }
      }
    }
